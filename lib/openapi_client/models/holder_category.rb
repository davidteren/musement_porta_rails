...
module OpenapiClient
  class HolderCategory
    # When `true` the holder category appears first in lists.  If no holder category is defined as the default, then the holder category with a `holder_type` value of `ADULT` is automatically made the default. If no adult holder categories are available, then the first holder category is automatically made the default. 
    attr_accessor :default_category

    # The holder category ID, assigned by the supplier.
    attr_accessor :holder_category_id

    # The type of holder category.  Please note that usage of the `CUSTOM*` holder type requires approval from the PORTA team. 
    attr_accessor :holder_type

    # The maximum age customers can have to qualify for the holder category.
    attr_accessor :maximum_age

    # The minimum age customers must have to qualify for the holder category.
    ...
    def self.attribute_map
      {
        ...
        :'holder_category_id' => :'holder_category_id',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::HolderCategory` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::HolderCategory`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'holder_category_id')
        self.holder_category_id = attributes[:'holder_category_id']
      else
        self.holder_category_id = nil
      end

      if attributes.key?(:'holder_type')
        self.holder_type = attributes[:'holder_type']
      else
        self.holder_type = nil
      end
    ...
    def list_invalid_properties
      ...
      if @holder_category_id.nil?
        invalid_properties.push('invalid value for "holder_category_id", holder_category_id cannot be nil.')
      end

      pattern = Regexp.new(/^(?!\-|\.|\_)[0-9a-z\-\.\_]{1,50}$/)
      if @holder_category_id !~ pattern
        invalid_properties.push("invalid value for \"holder_category_id\", must conform to the pattern #{pattern}.")
      end
      ...
      if @minimum_age.nil?
        invalid_properties.push('invalid value for "minimum_age", minimum_age cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @holder_category_id.nil?
      return false if @holder_category_id !~ Regexp.new(/^(?!\-|\.|\_)[0-9a-z\-\.\_]{1,50}$/)
      return false if @holder_type.nil?
      holder_type_validator = EnumAttributeValidator.new('String', ["ADULT", "CHILD", "CUSTOM*", "FAMILY", "GROUP", "INFANT", "MILITARY", "REGULAR", "SENIOR", "STUDENT", "YOUTH"])
      return false unless holder_type_validator.valid?(@holder_type)
      return false if @minimum_age.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] holder_category_id Value to be assigned
    def holder_category_id=(holder_category_id)
      if holder_category_id.nil?
        fail ArgumentError, 'holder_category_id cannot be nil'
      end

      pattern = Regexp.new(/^(?!\-|\.|\_)[0-9a-z\-\.\_]{1,50}$/)
      if holder_category_id !~ pattern
        fail ArgumentError, "invalid value for \"holder_category_id\", must conform to the pattern #{pattern}."
      end
    ...
    def holder_type=(holder_type)
      validator = EnumAttributeValidator.new('String', ["ADULT", "CHILD", "CUSTOM*", "FAMILY", "GROUP", "INFANT", "MILITARY", "REGULAR", "SENIOR", "STUDENT", "YOUTH"])
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          holder_category_id == o.holder_category_id &&
    ...
    def hash
      [default_category, holder_category_id, holder_type, maximum_age, minimum_age].hash
    end
