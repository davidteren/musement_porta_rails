...
module OpenapiClient
  class TimeSlot
    ...
    # A list of IDs for pickups available for this time slot.
    attr_accessor :pickup_ids

    # The date and time the experience starts.  The value for this property must be in the local time for the experience with no time zone information, following the [ISO 8601 standard](https://www.iso.org/iso-8601-date-and-time-format.html). 
    attr_accessor :start
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::TimeSlot` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::TimeSlot`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'pickup_ids')
        if (value = attributes[:'pickup_ids']).is_a?(Array)
          self.pickup_ids = value
        end
      end
    ...
    # @param [Object] pickup_ids Value to be assigned
    def pickup_ids=(pickup_ids)
      if pickup_ids.nil?
        fail ArgumentError, 'pickup_ids cannot be nil'
      end
