...
module OpenapiClient
  class Pickup
    ...
    # The number of minutes a pickup will occur before or after a time slot. This property accepts both positive and negative values.  A negative value indicates the pickup arrives *before* the time slot.  A positive value indicates the pickup arrives *after* the time slot. 
    attr_accessor :time_margin
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::Pickup` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::Pickup`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'time_margin')
        self.time_margin = attributes[:'time_margin']
      else
        self.time_margin = 0
      end
    end
    ...
    def list_invalid_properties
      ...
      if @pickup_id.nil?
        invalid_properties.push('invalid value for "pickup_id", pickup_id cannot be nil.')
      end
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["HOTEL", "MEETING_POINT"])
      return false unless type_validator.valid?(@type)
      return false if !@latitude.nil? && @latitude > 90
      return false if !@latitude.nil? && @latitude < -90
      return false if !@longitude.nil? && @longitude > 180
      return false if !@longitude.nil? && @longitude < -180
      return false if @pickup_id.nil?
      return false if @pickup_id !~ Regexp.new(/^(?!\-|\.|\_)[0-9a-z\-\.\_]{1,50}$/)
      true
    end
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["HOTEL", "MEETING_POINT"])
    ...
    def hash
      [name, type, latitude, longitude, pickup_id, time_margin].hash
    end
