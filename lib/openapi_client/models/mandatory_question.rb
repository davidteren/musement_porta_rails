  ...
  class MandatoryQuestion
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::MandatoryQuestion` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::MandatoryQuestion`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'data_pattern')
        self.data_pattern = attributes[:'data_pattern']
      end
      ...
      if attributes.key?(:'select')
        if (value = attributes[:'select']).is_a?(Hash)
          self.select = value
        end
      end
    end
    ...
    def valid?
      ...
      data_type_validator = EnumAttributeValidator.new('String', ["DATE", "DECIMAL", "EMAIL", "INTEGER", "PHONE_NUMBER", "SELECT_ONE", "STRING"])
      return false unless data_type_validator.valid?(@data_type)
      return false if @level.nil?
      level_validator = EnumAttributeValidator.new('String', ["BOOKING", "BY_PAX"])
    ...
    def data_type=(data_type)
      validator = EnumAttributeValidator.new('String', ["DATE", "DECIMAL", "EMAIL", "INTEGER", "PHONE_NUMBER", "SELECT_ONE", "STRING"])
    ...
    def level=(level)
      validator = EnumAttributeValidator.new('String', ["BOOKING", "BY_PAX"])
    ...
    def hash
      [data_pattern, data_type, holder_category_id, level, mandatory_question_id, option_id, question, select].hash
    end
