...
module OpenapiClient
  class TestHoldAvailabilityRequest
    # The ID for an existing sandbox experience.
    attr_accessor :experience_id

    # When the hold availability request can expire, based on the agreed upon `hold_duration` value.  The value for this property must be in the local time for the experience with no time zone information, following the [ISO 8601 standard](https://www.iso.org/iso-8601-date-and-time-format.html). 
    ...
    def self.openapi_types
      {
        ...
        :'option_id' => :'String',
        :'pickup_id' => :'String',
        :'quantities' => :'Array<TestHoldAvailabilityRequestQuantitiesInner>',
        :'slot_id' => :'String'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::TestHoldAvailabilityRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::TestHoldAvailabilityRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @quantities.nil?
        invalid_properties.push('invalid value for "quantities", quantities cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @quantities.nil?
    ...
    # Custom attribute writer method with validation
    # @param [Object] quantities Value to be assigned
    def quantities=(quantities)
      if quantities.nil?
        fail ArgumentError, 'quantities cannot be nil'
      end
    ...
    def hash
      [experience_id, expiration_time, hold_id, option_id, pickup_id, quantities, slot_id].hash
    end
